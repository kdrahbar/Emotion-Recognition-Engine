...
void frame::calc_centroids()
{
	...
	for (int i = 5; i < 11; i++)
	{   cout << l_eye_region_landmks[i].x << "," << l_eye_region_landmks[i].y << "," <<
	              r_eye_region_landmks[i].x << "," << r_eye_region_landmks[i].y << ",";
		lx_sum += l_eye_region_landmks[i].x;
		ly_sum += l_eye_region_landmks[i].y;
		rx_sum += r_eye_region_landmks[i].x;
		ry_sum += r_eye_region_landmks[i].y;
	}
	cout << endl;
...
void frame::extract_eye_region_landmarks()
{
	int r_tmp [11] = {5,6,7,8,9,25,26,27,28,29,30};
	int l_tmp [11] = {0,1,2,3,4,19,20,21,22,23,24};
	for (int i = 0; i < 11; i++)
	{
		l_eye_region_landmks.push_back(landmarks[l_tmp[i]]);
		r_eye_region_landmks.push_back(landmarks[r_tmp[i]]);
	}
}
...
// Constructor that builds the landmark vector given opencv matrix
frame::frame(cv::Mat cvLandmarks)
{
	point add_point;
	for( int i = 0; i < cvLandmarks.cols; i++ )
	{
		add_point.x = (int)cvLandmarks.at<float>(0,i);
		add_point.y = (int)-cvLandmarks.at<float>(1,i);
		landmarks.push_back(add_point);
	}
	//create_brow_landmarks();
}
...
std::vector<float> frame::eyebrow_feat_ratio(std::vector<float> peek_feat, std::vector<float> neutral_feat)
{
	std::vector<float> eyebrow_feat;
	float temp;
	for (int i = 0; i < 6; i++)
	{
		if(i == 2 || i == 3)
		{
			...
			eyebrow_feat.push_back(temp);
		}
		else
		{
			temp = (peek_feat[i]-neutral_feat[i])/neutral_feat[i] * 100;
			eyebrow_feat.push_back(temp);
		}
	}
...
std::vector<float> frame::extract_eye_feat(point neutral_ctrd, std::vector<point> lndmarks)
{
	float angle = atan( (lndmarks[5].y - lndmarks[8].y)/(lndmarks[5].x - lndmarks[8].x) );
	//cout << angle;
	lndmarks = matrix_mult(angle, lndmarks);
...
float frame::get_probability(float feat, float mean, float std)
{
...
std::vector<float> frame::calc_probs(std::vector<float> model_vals, std::vector<float> feat_vals)
{
	...
	int lin_count = 0; // start at two since the first two values from the bayes vector are the class probs
	...
	for(int i = 1; i < itr_size/2 ; i+=2)
	{
			//cout << i << "," << lin_count << endl;
			prob_a += log(get_probability(feat_vals[lin_count], model_vals[i], model_vals[i+1]));
			prob_b += log(get_probability(feat_vals[lin_count], model_vals[i+itr_size/2], model_vals[i+itr_size/2 +1]));
			lin_count++;
	}
